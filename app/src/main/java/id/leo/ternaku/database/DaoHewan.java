...
public interface DaoHewan {
    ...
    @Query("SELECT count(*) FROM tb_profil")
    int checkTableProfil();

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    void insertDataProfil (TabelProfil ... tabelProfils);

    @Query("SELECT * FROM tb_profil WHERE id = 1")
    List<TabelProfil> getFirstDataProfil();
    @Update
    void updateProfil (TabelProfil tabelProfil);
}
